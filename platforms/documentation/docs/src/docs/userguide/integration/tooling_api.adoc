...
[[sec:tapi_version_info]]
=== Getting CLI help and version info

The Tooling API exposes lightweight models that return the exact command-line text quickly, without starting a Gradle daemon:

* link:{javadocPath}/org/gradle/tooling/model/build/VersionInfo.html[org.gradle.tooling.model.build.VersionInfo] — the banner printed by `gradle --version`.
* link:{javadocPath}/org/gradle/tooling/model/build/Help.html[org.gradle.tooling.model.build.Help] — the full output printed by `gradle --help`.

You can fetch these models like any other:

[source,java]
----
try (var connection = GradleConnector.newConnector().forProjectDirectory(projectDir).connect()) {
    // Example of fetching the Version model
    var versionInfo = connection.getModel(org.gradle.tooling.model.build.VersionInfo.class);
    System.out.println(versionInfo.getVersionOutput());

    // Example of fetching the Help model
    var help = connection.getModel(org.gradle.tooling.model.build.Help.class);
    System.out.println(help.getRenderedText());
}
----

For convenience, the existing link:{javadocPath}/org/gradle/tooling/model/build/BuildEnvironment.html#getVersionInfo()[BuildEnvironment#getVersionInfo()] also returns the same banner text when you already query the `BuildEnvironment` model.

Additionally, when you execute operations via the Tooling API consumer and pass CLI flags, the following flags are honored the same way as on the command line:

* `-?`, `-h`, `--help` — print help and short-circuit the operation.
* `-v`, `--version` — print the version banner and short-circuit the operation.
* `-V`, `--show-version` — print the version banner and continue with the operation.

When targeting older Gradle versions that do not provide these models or methods, the Tooling API will throw the usual
link:{javadocPath}/org/gradle/tooling/UnknownModelException.html[UnknownModelException] for unknown models and
link:{javadocPath}/org/gradle/tooling/UnsupportedVersionException.html[UnsupportedVersionException] for unsupported methods.
Feature‑detect and fall back as needed.
